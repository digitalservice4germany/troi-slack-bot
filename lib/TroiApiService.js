...
const AuthenticationFailed = require('./AuthenticationFailed');
...
module.exports = class TroiApiService {
  ...
  async getCalculationPositions() {
    const calculationPositions = await this.makeRequest({
      ...
      params: {
        clientId: this.clientId,
        favoritesOnly: "true", // otherwise there are hundreds of results, offer as backup option if someone didn't set favorites?
      },
    });
